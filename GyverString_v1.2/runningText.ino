...
void drawLetter(int index, int letter, int offset, byte color) {
  ...
  // ДЕЛАЕМ 32-битный цвет
  uint32_t letterColor;

  if (color == 1) {
    // Радуга по смещению
    letterColor = mHSV(byte(offset * 10), 255, 255);
  } else if (color == 2) {
    // Разноцветные буквы
    letterColor = mHSV(byte(index * 30), 255, 255);
  } else {
    // Статический цвет
  if (useRGB) {
    // Гайверовский формат цвета, всё сам сделает как надо
    letterColor = mRGB(rgbR, rgbG, rgbB);
  } else {
    letterColor = mHSV(thisColor, 255, 255);
  }
